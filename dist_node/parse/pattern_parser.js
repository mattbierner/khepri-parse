...
(arrayPattern = label("Array Pattern", ((element = topLevelPattern), (elements = sepBy1(sep, expected(
    "array pattern element", element))), node(between(punctuator("["), punctuator("]"), eager(elements)),
...
(asPattern = label("As Pattern", nodea(enumeration(attempt(then(identifierPattern, punctuator("#"))), expected(
    "object or array pattern", either(arrayPattern, objectPattern))), ast_pattern.AsPattern.create)));
...
(argumentsPattern = label("Arguments Pattern", either(nodea(enumeration(attempt(then(optional(null, identifierPattern),
