...
define(["require", "exports", "nu-stream/stream", "bennu/parse", "bennu/lang", "khepri-ast/clause",
    ...
    var x3 = label.bind(null, "Static Statement"),
        ...
        y4 = node(between(keyword("static"), logicalSemiColon, staticDeclarationList), ast_declaration.StaticDeclaration
            .create);
    (staticStatement = x3(y4));
    var variableDeclarationList = ((declaratorId = either(identifier, operator)), (initializer = enumeration(
        punctuator("=", ":=", "=:"), ((y5 = expected.bind(null, "variable initilizer")), y5(expression))
    )), (variableDeclaration = nodea(cons(declaratorId, optional(NIL, initializer)), (function(loc, id, op,
        init) {
        ...
        y6 = node(between(keyword("var"), logicalSemiColon, variableDeclarationList), ast_declaration.VariableDeclaration
            .create);
    ...
    var y9, x5 = label.bind(null, "With Statement"),
        ...
        y8 = nodea(enumeration(between(keyword("with"), keyword("in"), ((y9 = expected.bind(null,
            "with bindings")), y9(bindings))), blockStatement), ast_statement.WithStatement.create);
    (withStatement = x5(y8));
    var y10, x6 = label.bind(null, "If Statement"),
        test = between(punctuator("("), punctuator(")"), ((y10 = expected.bind(null, "if condition")), y10(
            expression))),
        alternate = next(keyword("else"), statement),
        y11 = nodea(next(keyword("if"), enumeration(test, statement, optional(alternate))), ast_statement.IfStatement
            .create);
    (ifStatement = x6(y11));
    var y12, y14, x7 = label.bind(null, "Switch Statement"),
        caseClause = nodea(enumeration(between(keyword("case"), punctuator(":"), ((y12 = expected.bind(null,
            "case test")), y12(expression))), statementList), ast_clause.SwitchCase.create),
        ...
        caseBlock = eager(between(punctuator("{"), punctuator("}"), append(optional(NIL, caseClauses), optional(
            NIL, enumeration(defaultClause))))),
        y13 = nodea(next(keyword("switch"), enumeration(between(punctuator("("), punctuator(")"), ((y14 =
                expected.bind(null, "switch discriminant")), y14(expression))), caseBlock)), ast_statement.SwitchStatement
            .create);
    (switchStatement = x7(y13));
    var whileStatement = ((x8 = label.bind(null, "While Statement")), (condition = between(punctuator("("),
        punctuator(")"), expression)), (y15 = nodea(next(keyword("while"), enumeration(condition, statement)),
        ast_statement.WhileStatement.create)), x8(y15)),
        doWhileStatement = ((x9 = label.bind(null, "Do While Statement")), (condition0 = between(punctuator("("),
            punctuator(")"), expression)), (y16 = nodea(next(keyword("do"), enumeration(blockStatement,
                between(keyword("while"), logicalSemiColon, condition0))), ast_statement.DoWhileStatement
            .create)), x9(y16)),
        forStatement = ((x10 = label.bind(null, "For Statement")), (forInitExpression = either(node(next(
                keyword("var"), variableDeclarationList), ast_declaration.VariableDeclaration.create),
            topLevelExpression)), (y17 = nodea(next(keyword("for"), enumeration(next(punctuator("("),
                optional(forInitExpression)), next(logicalSemiColon, optional(expression)),
            next(logicalSemiColon, optional(topLevelExpression)), next(punctuator(")"),
                statement))), ast_statement.ForStatement.create)), x10(y17)),
    ...
    var x14 = label.bind(null, "Return Statement"),
        y21 = node(between(keyword("return"), logicalSemiColon, optional(expression)), ast_statement.ReturnStatement
            .create);
    ...
    var x16 = label.bind(null, "Try Statement"),
        catchBlock = nodea(next(keyword("catch"), enumeration(between(punctuator("("), punctuator(")"),
            identifier), blockStatement)), ast_clause.CatchClause.create),
        finallyBlock = next(keyword("finally"), blockStatement),
        y23 = nodea(next(keyword("try"), enumeration(blockStatement, optional(catchBlock), optional(
            finallyBlock))), ast_statement.TryStatement.create);
    (tryStatement = x16(y23));
    var x17 = label.bind(null, "Statement"),
        y24 = choice(blockStatement, staticStatement, variableStatement, emptyStatement, ifStatement,
            withStatement, iterationStatement, continueStatement, breakStatement, returnStatement,
            switchStatement, throwStatement, tryStatement, debuggerStatement, expressionStatement),
        x18 = x17(y24),
