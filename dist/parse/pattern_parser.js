...
*/define(["require", "exports", "bennu/parse", "bennu/lang", "nu-stream/stream", "khepri-ast/pattern", "./common",
    ...
    var attempt = __o["attempt"],
        ...
        pattern, topLevelPattern, identifierPattern, sinkPattern, ellipsisPattern, importPattern, arrayPattern,
            objectPattern, argumentList, argumentsPattern, asPattern, element, pre, post, sepEndWith1 = (
                function(sep, end, p) {
    ...
    (ellipsisPattern = label("Ellipsis Pattern", node(next(punctuator("..."), identifierPattern), ast_pattern.EllipsisPattern
        .create)));
    (arrayPattern = label("Array Pattern", ((element = topLevelPattern), (pre = sepEndWith(sep, ellipsisPattern,
        expected("array pattern element", element))), (post = sepBy(sep, expected(
        "non-ellipsis array pattern element", element))), node(between(punctuator("["), punctuator(
        "]"), eager(append(pre, next(sep, post)))), ast_pattern.ArrayPattern.create))));
